...
export default async function Layout({ children, params }: Props) {
  ...
  return (
    <div className="container relative hidden h-screen flex-col items-center justify-center md:grid lg:max-w-none lg:grid-cols-2 lg:px-0">
      {/* {page === "/auth/[applicationId]/sign-up" && (
        <Button
          asChild
          variant="ghost"
          className="absolute right-4 top-4 md:right-8 md:top-8"
        >
          <Link href={`/auth/${applicationId}/sign-in`}>Login</Link>
        </Button>
      )}

      {page === "/auth/[applicationId]/sign-in" && (
        <Button
          asChild
          variant="ghost"
          className="absolute right-4 top-4 md:right-8 md:top-8"
        >
          <Link href={`/auth/${applicationId}/sign-up`}>Register</Link>
        </Button>
      )}

      {page === "/auth/[applicationId]/forgot-password" && (
        <Button
          asChild
          variant="ghost"
          className="absolute right-4 top-4 md:right-8 md:top-8"

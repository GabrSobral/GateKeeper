...
<Sheet.Root>
		...
		<Card.Root class="transition-all hover:scale-[1.01] hover:cursor-pointer hover:shadow-lg">
			...
			<Card.Content class="flex gap-1">
				<Badge variant="outline" class="w-fit">
					{draftClientId && draftClientSecret ? 'Configured' : 'Not configured'}
				</Badge>

				<Badge variant={draftIsEnabled ? 'default' : 'secondary'} class="w-fit">
	...
	<Sheet.Content side="right">
		<Sheet.Header>
			...
			<Sheet.Description>
				Make changes to your Google authentication provider configuration. Then click "Save changes"
				to apply them.
			</Sheet.Description>
		</Sheet.Header>
